...
def requirements(task = None, env = None):
    """
    It takes a task as an argument, and if the task is generate, it generates a requirements.back.txt
    file, if the task is update, it updates the requirements.back.txt file, and if the task is install,
    it installs the requirements.back.txt file
    ...
    if task in ['generate','update']:
        ...
        #pip freeze > Requirements.back.txt
        subprocess.call('pip install -r Requirements.back.txt', shell=True)
        
        if task == 'generate':
            log.info(f'Requirements.back.txt file generated on current {env} environment')
            log.info(f'Project requirements handled successfully for {env} environment')
            
        elif task == 'update':
            log.info(f'Requirements.back.txt file updated on current {env} environment')
            log.info(f'Project requirements handled successfully for {env} environment')
            
    elif task == 'install':
        
        try:
            # try to install dependencies from the requirements.back.txt file
            log.info('Installing requirements ...')
            
            subprocess.call('pip install -r Requirements.back.txt', shell=True)
            
            log.info(f'Requirements.back.txt file installed successfully on current {env} environment')
            log.info(f'Project requirements handled successfully for {env} environment')
        except Exception as exception:
            log.error(f'Error while installing dependencies for {env} environment')
            log.error(exception) 
            log.error("If there is an error at this stage, it means that the python environment has some incompatibilities with the requirements.back.txt packages")
        ...
        log.error('Requirements.back.txt file not generated')
...
def requirements_main(env = None):
    ...
    requirements.back.txt file, if the file already exists, the function will update the file. 
    
    If the environment variable is set to production or prod, the function will install the dependencies
    from the requirements.back.txt file. 
    ...
    try:
        ...
        if current_env in ['development','dev']:
            ...
            if os.path.exists('Requirements.back.txt'):
                # remove previous requirements.back.txt file 
                os.remove('Requirements.back.txt')
                requirements('update', current_env)
                
            else:
                # create a new requirements.back.txt file
                requirements('generate', current_env)
                
        elif current_env in  ['production','prod']:
            #install dependencies from the requirements.back.txt file
        ...
        log.error('Requirements.back.txt could not be handled')
